...
class Dungeon {
  ...
  _targetableEnemies() {
    ...
    const dv = this.party.some(p => !p.left && p.hp > 0 && (p.darkvision || /^darkvision/.test(p.vision || '') || p.blindsense > 0 || p.trueSeeing || p.seeInvis)), dvDeep = this.party.some(p => !p.left && p.hp > 0 && (p.blindsense > 0 || p.trueSeeing));   // v3.37.154: DEEPER darkness yields only to True Seeing / blindsense
  ...
  openDoor() {
    ...
    for (const m of this.present()) { this._computeCastable(m); this._resetAbilities(m); this._stanceDoorNote(m); m.flatFooted = !(fighterFeats(m.cls, m.level, this._isRanged(m)).supremacy || (this._isFlameCavalier(m) && (m.level || 1) >= 2) || this._twkActive(m, 'lookout') || m.foresight); }   // LOOKOUT (teamwork): a paired watch is never surprised  // re-read the spell LOADOUT (Spellbook picker edits land at the door) + refresh per-room spells/channels + flat-footed until they act (Weapon Supremacy — and Order of the Flame's FOOLHARDY RUSH at L2 — are never caught flat-footed)
  ...
  _swingVsAC(attacker, ac, target, extraToHit = 0, offHand = false) {
    ...
    if ((arcHoly || attacker._holySword) && (target.evil || target.markedEvil)) dmg += dRollN(Math.max(arcHoly, attacker._holySword || 0), 6);   // Holy Sword (paladin 4th, v3.37.140) blesses whatever weapon is in hand
