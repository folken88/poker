  ...
  function renderDungeon() {
    ...
    if (party) party.innerHTML = (d.party || []).map(m => {
      ...
      return `<div class="${cls.join(' ')}" style="position:relative">${kickHtml}
        <div class="dpc__ac" title="Armor Class — current total" style="position:absolute;bottom:3px;right:5px;font-size:0.7rem;font-weight:700;color:var(--brass-bright);background:rgba(0,0,0,0.55);border-radius:6px;padding:0 5px;line-height:1.45;pointer-events:none;z-index:6">🛡 ${Number.isFinite(m.ac) ? m.ac : '?'}</div>
